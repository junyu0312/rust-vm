...
pub fn create_kvm_vm(
    ...
    vm.init_vcpus(cpus).context("Failed to create vcpus")?;

    vm.init_mm(memory_size_bytes).context("Failed to init mm")?;
    ...
    let bz_image = BzImage::new(kernel, initramfs, cmdline)?;
